...
class QwenImagePipeline(BasePipeline):
    ...
    def clear_lora(self):       
        """清除模型中所有AutoWrappedLinear模块的LoRA权重。
        
        此方法会遍历模型中的所有模块，找到AutoWrappedLinear类型的模块，
        并清空其lora_A_weights和lora_B_weights列表，从而实现LoRA卸载。
        """
        cleared_count = 0
    ...
    def reload_original_weights(self, model_configs: list[ModelConfig] = []):
        """重新加载原始模型权重，移除通过GeneralLoRALoader直接修改的LoRA。
        
        Args:
            model_configs: 模型配置列表，与from_pretrained方法中的参数相同。
                如果未提供，则使用当前加载的模型配置。
        """
        # 记录当前设备和数据类型
        device = self.device
        ...
        # 更新当前pipeline的模型
        self.text_encoder = model_manager.fetch_model("qwen_image_text_encoder")
        self.dit = model_manager.fetch_model("qwen_image_dit")
        self.vae = model_manager.fetch_model("qwen_image_vae")
        
        # 重新创建blockwise_controlnet
        try:
            self.blockwise_controlnet = QwenImageBlockwiseMultiControlNet(model_manager.fetch_model("qwen_image_blockwise_controlnet", index="all"))
        except Exception:
            # 如果不存在controlnet，则跳过
            pass
        
        # 如果已启用VRAM管理，重新应用
        if hasattr(self, 'vram_management_enabled') and self.vram_management_enabled:
            self.enable_vram_management()
            
        print("已重新加载原始模型权重，LoRA已卸载")
